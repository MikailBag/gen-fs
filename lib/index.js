...
let context = Context.compile.modules({'fs':'fs',http: 'http',https: 'https',dns: 'dns',util: './util'});
...
module.exports = exports = function exports(generator, errCallback, sync) {
    ...
    let gen = generator(context);
    let result;
    let errback = errCallback ? errCallback : function (err) {
        gen.throw(err);
    };
    //
