...
Context.compile = function (obj) {
    ...
    log.verbose(`Context.compile(): compiling objrct ${util.inspect(JSON.stringify(obj))}`);
    var ctx = {};
    ...
    for (let prop in obj) {
        ...
        var type = typeof val;
        if (val == obj) {
            log.verbose(`skipping ${prop}, because it is circular`);
            continue;
        }
        if (type == 'string' || type == 'number') {
            log.verbose(`skipping ${prop}, because it is of primitive type`);
            continue;
        }
        //TODO skip classes
        log.verbose('compiling %s, it\'s type is %s', prop, type);
        if (type === 'function') {
            ctx[prop] = Context.compile.func(val,prop);
        } else if (type === 'object') {
...
Context.compile.func = function (func,name) {
